...
export default function GenericCampaignAdminEditor({
  reportId,
  initialData,
  title: initialTitle,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-card border border-va-border bg-va-surface p-6 shadow-sm">
        <h3 className="font-heading text-lg font-semibold text-va-navy mb-4">Campaign info</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block mb-1.5 text-sm font-body font-medium text-va-text-secondary">Context</label>
            <p className="px-3 py-2 text-sm font-body text-va-text-secondary bg-va-surface-2 rounded-card border border-va-border">
              {serviceLabel(serviceType)} · {campaignType === 'event' ? `Event: ${eventName ?? '—'}` : 'General'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-card border border-va-border bg-va-surface p-6 shadow-sm">
        <div className="mb-4">
          ...
          <p className="text-xs font-body text-va-text-muted mt-0.5">
            No bespoke editor exists for this service block yet. Paste or edit the JSON directly.
          </p>
        </div>
        <textarea
          value={rawJson}
      ...
      <div className="flex items-center justify-between sticky bottom-4 rounded-card border border-va-border bg-va-surface/95 backdrop-blur-sm p-4 shadow-lg">
        <p className="text-sm font-body text-va-text-muted">
          Changes save to the client&apos;s campaign.
        </p>
