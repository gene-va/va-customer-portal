...
interface ReportHeaderProps { title: string; updatedAt: string; generatedDate?: string; }

export default function ReportHeader({ title, updatedAt, generatedDate }: ReportHeaderProps) {
  ...
  return (
    <div className="sticky top-0 z-20 border-b border-va-border bg-va-surface/90 backdrop-blur-sm no-print">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        <Link href="/dashboard" className="flex items-center gap-2 text-va-navy hover:text-va-blue flex-shrink-0 transition-colors">
          <ArrowLeft size={18} />
          <span className="text-sm font-body font-medium hidden sm:inline">Dashboard</span>
        </Link>
